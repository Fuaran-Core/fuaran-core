module Fuaran.Core.Tests.UiIdlSupport
...
// ---------------------------------------------------------------------------
// The Switch vector constraint — the cross-field rule the IDL cannot state.
//
// The wire contract for `Switch` is: EXACTLY ONE of `on` / `stateKey`, with the
...
// input no conformant encoder can produce, while the IDL interpreter echoes it.
//
// This rewrite narrows a sampled vector to the canonical space, in the same
// deterministic way for every leg. It is declared HERE, beside the projection
// that necessitates it, so the narrowing is visible support-data rather than a
// test-side patch — and so retiring the projection retires it.
// ---------------------------------------------------------------------------

/// Rewrite one sampled Switch field set to the canonical `on`/`stateKey` spelling.
let private canonicaliseSwitchFields (fields: (string * IdlValue) list) : (string * IdlValue) list =
    ...
    //    alongside the rewrite makes a duplicate key, and in the TS value
    //    literal the trailing `undefined` wins and silently swallows the real
    //    one (found the hard way: the interpreter leg agreed with itself while
    //    the TS leg dropped the field).
    let collapse =
        fields
...
/// Rewrite one sampled SetState field set to the value-XOR-valueFrom wire: the
/// same cross-field class as Switch — the refine refuses both-present and
...
/// Narrow a sampled vector to the canonical Switch wire — see the note above.
let rec canonicaliseVector (v: IdlValue) : IdlValue =
    ...
    // markers): nothing to recurse into, nothing Switch-shaped to rewrite.
    | _ -> v
