...
# fuaran-core — the cross-pipeline VALUE probe.
#
# WHY THIS IS SEPARATE FROM ./verify.ps1. That gate's Fable step is a COMPILE gate: it proves every
# public package transpiles, which is a different claim from "and computes the same number". Nothing
# in a .NET test suite can make the second claim either. `Hash.fnv1a` sat divergent between the two
# pipelines behind a fully green 719-test suite until 0.6.0 — measured, not hypothetical: with the
# naive multiply reintroduced, the suite stays green while 120 of this corpus's 124 entries diverge.
# So this is the only check in the repo that can catch that class, and it is the one to run when a
# value is claimed to be portable.
#
# It is NOT wired into ./run.ps1 or ./verify.ps1 on purpose: it needs a Node runtime, and the default
# gate is deliberately dependency-free. Run it by hand after touching `Hash.mul32`, `Hash.(.+.)`, or
# anything else whose value has to agree across pipelines.
#
...
$dotnetOut = (dotnet run --project HashParityProbe.fsproj) | Where-Object { $_ -match '^\d{3} ' }
...
dotnet fable HashParityProbe.fsproj -o $outDir --noCache | Out-Null
